...
def test_evaluate_coulomb_energy_two_particle():
    ...
    global_parameters = torch.tensor([scale_factor])

    actual_energy = evaluate_coulomb_energy(
        conformer, atom_indices, parameters, global_parameters
    )
...
def test_evaluate_coulomb_energy_three_particle():
    ...
    parameters = torch.tensor([[[q_a], [q_b]], [[q_a], [q_c]], [[q_b], [q_c]]])
    global_parameters = torch.tensor([scale_factor])
    ...
    expected_energy = torch.tensor(
        _COULOMB_PRE_FACTOR
        * scale_factor
        * (q_a * q_b / 4.0 + q_a * q_c / 3.0 + q_b * q_c / 5.0),
    )
...
def test_evaluate_lj_energy_two_particle():
    ...
    actual_energy = evaluate_lj_energy(
        conformer, atom_indices, parameters, global_parameters
    )
    expected_energy = (
        scale_factor
...
def test_evaluate_lj_energy_three_particle():
    ...
    eps_a, sigma_a = 0.1, 2.0
    eps_b, sigma_b = 0.3, 4.0
    eps_c, sigma_c = 0.5, 6.0

    parameters = torch.tensor(
        [
            [[eps_a, sigma_a], [eps_b, sigma_b]],
            [[eps_a, sigma_a], [eps_c, sigma_c]],
            [[eps_b, sigma_b], [eps_c, sigma_c]],
        ]
    )
    global_parameters = torch.tensor([scale_factor])
    ...
    expected_energy = torch.tensor(
        scale_factor * 4.0 * eps_ab * ((sigma_ab / 4.0) ** 12 - (sigma_ab / 4.0) ** 6)
        + scale_factor * 4.0 * eps_ac * ((sigma_ac / 3.0) ** 12 - (sigma_ac / 3.0) ** 6)
